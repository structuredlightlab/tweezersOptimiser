...
% See LICENSE.md for information about using/distributing this file.
  ...
  methods (Access=protected)
    ...
    function [A, B] = translateZ_type_helper_derivative(beam, z, Nmax, derivativeOrder)
       % This function is NOT part of the optical tweezers toolbox, but it was adapted from OTT's "translateZ_type_helper".
       % Author: Une B
      ...
      switch beam.basis
        case 'regular'
          translation_type = 'sbesselj';
        otherwise
              error('only regular beams supported')
      end
      
      % Calculate tranlsation matrices
      [A, B] = ott.utils.translate_z_derivative(Nmax, z, derivativeOrder, 'type', translation_type);
...
%       I = sum(abs(E).^2, 1);
    ...
    function varargout = visualiseFarfield(beam, varargin)
      ...
      p.addParameter('direction', 'pos');
      p.addParameter('field', 'E2');            %Une: changed from 'irradiance' to 'E2'
      p.addParameter('mapping', 'sin');
...
%           'of OTT not recomended, see documentation for more info']);
    ...
    function [beam, A, B] = translateZ(beam, varargin)
     ...
     function [beam, A, B] = translateZ_derivative(beam, derivativeOrder, varargin)
      ...
      if ~isempty(p.Results.z)
        ...
        for ii = 1:numel(z)
          [A, B] = ibeam.translateZ_type_helper_derivative(z(ii), [p.Results.Nmax, ibeam.Nmax], derivativeOrder);
          beam = beam.append(ibeam.translate(A, B));
          beam.basis = 'regular';
        end
      else
    ...
    function varargout = translateXyz_derivative(beam, derivativeOrder, varargin)
      % This function is NOT part of the optical tweezers toolbox, but it was adapted from OTT's "translateXyz".
      % Author: Une B
      % Calculate a derivative of a translation matrix, given Cartesian coordinates.
      p = inputParser;
      p.addOptional('opt1', []);    % xyz or Az
      p.addOptional('opt2', []);    % [] or Bz
      p.addOptional('opt3', []);    % [] or D
      ...
      if ~isempty(p.Results.opt1) && isempty(p.Results.opt2) ...
        ...
        xyz = p.Results.opt1;
        rtp = ott.utils.xyz2rtp(xyz.').';
        [varargout{1:nargout}] = beam.translateRtp_derivative(derivativeOrder, rtp, ...
            'Nmax', p.Results.Nmax);
      else
        [varargout{1:nargout}] = beam.translateRtp_derivative(derivativeOrder, varargin{:});
      end
    end
    ...
    function [beam, A, B, D] = translateRtp(beam, varargin)
     ...
     function [beam, A, B, D] = translateRtp_derivative(beam, derivativeOrder, varargin)
      % This function is NOT part of the optical tweezers toolbox, but it was adapted from OTT's "translateRtp".
      % Author: Une B
      ...
      if any((theta ~= 0 & abs(theta) ~= pi) | phi ~= 0)
        ...
        for ii = 1:numel(r)
          [tbeam, D] = ibeam.rotateYz(theta(ii), phi(ii), ...
              'Nmax', max(oNmax, ibeam.Nmax));
          [tbeam, A, B] = tbeam.translateZ_derivative(derivativeOrder, r(ii), 'Nmax', oNmax);
          beam = beam.append(tbeam.rotate('wigner', D'));
        end
      else
        dnmax = max(oNmax, beam.Nmax);
        ...
        if numel(r) == 1
          [beam, A, B] = beam.translateZ_derivative(derivativeOrder, r, 'Nmax', oNmax);
        else
          beam = beam.translateZ_derivative(derivativeOrder, r, 'Nmax', oNmax);
        end
      end
    ...
    function beam = mtimes(a,b)
     ...
     function beam = times(a,b)
      % Une wrote this function
      %TIMES (op) multiply composite structure of the beam coefficients by
      %a vector
